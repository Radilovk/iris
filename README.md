...
По подразбиране OpenAI използва `gpt-4o`, а Gemini – `gemini-1.5-pro`. Можете да зададете друг модел чрез `AI_MODEL_EXTENDED`:

```toml
[vars]
AI_MODEL_EXTENDED = "gpt-4o-mini" # пример
```

При задаване на `AI_MODEL` той има приоритет и изцяло замества горната стойност.
