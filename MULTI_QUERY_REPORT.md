...
**Брой AI заявки:**
- **Single-query режим:** 3 заявки (2 за анализ на очите + 1 за доклад)
- **Multi-query режим:** 6 заявки (2 за анализ на очите + 4 за доклад)

Информацията за броя заявки се записва автоматично в `_analytics.ai_queries` секцията на всеки генериран доклад.
...
## Разлики в токени и брой заявки

### Стар подход (single-query режим):
**Брой AI заявки:**
- 2 заявки за анализ на изображенията (ляво и дясно око)
- 1 заявка за генериране на доклад
- **Общо: 3 заявки**
...
### Нов подход (multi-query режим):
**Брой AI заявки:**
- 2 заявки за анализ на изображенията (ляво и дясно око)
- 4 заявки за генериране на доклад (по една за всяка стъпка):
  1. Конституционална синтеза
  2. Интерпретация на знаците
  3. Персонализирани препоръки
  4. Финално сглобяване
- **Общо: 6 заявки**
...
**Забележка:** Въпреки двойното увеличение на броя заявки (от 3 на 6) и подобния общ брой токени, качеството на изхода е значително по-високо поради фокусирания контекст във всяка стъпка. Броят на заявките се записва автоматично в `_analytics.ai_queries` секцията на генерирания доклад.
