...
test('Изборът OpenAI/gpt-4o-mini се подава към API', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    assert.equal(body.messages[0].content[2].text, JSON.stringify({ eye: 'left' }));
    assert.equal(body.messages[0].content[3].image_url.url, 'data:image/png;base64,b');
    assert.equal(body.messages[0].content[4].text, JSON.stringify({ eye: 'right' }));
...
test('Изборът Gemini/gemini-1.5-flash се подава към API', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    assert.equal(body.contents[0].parts[2].text, JSON.stringify({ eye: 'left' }));
    assert.equal(body.contents[0].parts[3].inline_data.data, 'b');
    assert.equal(body.contents[0].parts[4].text, JSON.stringify({ eye: 'right' }));
